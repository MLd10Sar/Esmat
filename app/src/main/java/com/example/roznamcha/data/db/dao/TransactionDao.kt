...
""")
    ...
    // --- NEW QUERIES FOR COST OF GOODS SOLD ---
    @Query("SELECT SUM(costOfGoodsSold) FROM transactions WHERE category = 'SALE'")
    fun getTotalCostOfGoodsSold(): Flow<Double?>

    @Query("SELECT SUM(costOfGoodsSold) FROM transactions WHERE category = 'SALE' AND dateMillis BETWEEN :startDate AND :endDate")
...
""")
    ...
    WHERE category = 'SALE' AND paymentStatus = 'PAID' AND parentTransactionId IS NULL
""")
